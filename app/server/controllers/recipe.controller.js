    ...
    .then(promises => {
      ...
      for (let x = 0; x < promises[0].length; x++) {
        for (let y = 0; y < promises[1].length; y++) {
          if (
            promises[0][x].dataValues.recipeID ==
              promises[1][y].dataValues.recipeID &&
            promises[0][x].dataValues.count == promises[1][y].dataValues.count
          ) {
...
/*Recipe.findOne({
        ...
        if(recipe){
          ...
          res.json({msg: "Recipe could not be found"});
        }

        ingredientList.findAll({
          where: {
            recipeID: req.body.recipeID
          }
        }).then(result => {
